...
/**
 * 🎯 SOULFRA OUTCOMES PLATFORM (FIXED)
    ...
    function scheduleCall() {
      ...
      if (email) {
        ...
        fetch('/api/schedule', {
          ...
          body: JSON.stringify({ email, source: 'main_page' })
        });
      }
    }
    
    document.addEventListener('click', (e) => {
      if (e.target.classList.contains('big-button')) {
        fetch('/api/track', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  <script>
    ...
    function playTask(task, reward) {
      ...
      fetch('/api/game-complete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ task, reward, total: totalEarnings })
      });
...
<body>
    ...
    function scheduleDemo() {
      ...
      if (company) {
        ...
        fetch('/api/schedule-demo', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ company, type: 'business_assessment' })
        });
      }
    }
  </script>
  ...
  async handleScheduleDemo(req, res) {
    ...
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ status: 'demo_scheduled' }));
  }
